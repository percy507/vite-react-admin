...
export default function QuillEditor(props: QuillEditorProps) {
  ...
  const initConfigRef = useRef({ placeholder, value });
  ...
  useEffect(() => {
    ...
    const beforeCreateEditor = () => {
      const quillEditorEl = document.querySelector(`.${quillEditorClassNameRef.current}`);
      const toolbarElList = quillEditorEl?.querySelectorAll(
        `.${quillEditorClassNameRef.current} .ql-toolbar`,
      );
    ...
    const createEditor = () => {
      const { formats, modules } = editorConfig;
      const { value, placeholder } = initConfigRef.current;
      const _editor = new Quill(`.${editorContentClassNameRef.current}`, {
        // debug: 'info',
        formats,
        modules,
        placeholder,
  ...
  useEffect(() => {
    onChange(innerValue);
  }, [editor, innerValue, onChange]);

  console.log(innerValue);

  return (
    <div className={quillEditorClassNameRef.current}>
      <Spin spinning={uploadingImage}>
        <div
          className={editorContentClassNameRef.current}
          style={{
